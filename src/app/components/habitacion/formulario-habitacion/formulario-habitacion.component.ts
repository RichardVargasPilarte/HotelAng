  ...
  constructor(private habitacionServicio: HabitacionService,
    ...
    createForm(id?: string): void {
      if (this.data.type === 'c') {
        ...
        this.form = this.fb.group({
          id: this.data.hab!.id,
          nombre: new FormControl(this.data.hab!.nombre, [Validators.required, Validators.minLength(5)]),
          descripcion: new FormControl(this.data.hab!.descripcion, [Validators.required, Validators.minLength(15)]),
          precio: new FormControl(this.data.hab!.precio, [Validators.required, Validators.minLength(2)]),
          activo: new FormControl(this.data.hab!.activo, Validators.required),
          numero_personas: new FormControl(this.data.hab!.numero_personas, [Validators.required, Validators.minLength(1)]),
          nombre_alojamiento: new FormControl(this.data.hab!.nombre_alojamiento, Validators.required),
          nombre_alojamiento_id: new FormControl(this.data.hab!.nombre_alojamiento),
        });
      }
    }
    ...
    saveHabitacion(): void {
      ...
      this.subs.push(
        this.habitacionServicio.Agregar(hab)
          .subscribe(
            res => {this.dialogRef.close(); console.log(res); },
    ...
    updateHabitacion(): void {
      ...
      hab = Object.assign(hab, this.form.value);
      this.subs.push(
        this.habitacionServicio.ActualizarHabitacion(hab.id!, hab)
          .subscribe(
